...
*   **Purpose**: The service handles the *macro-planning and generation phase* of job lines. It creates a diverse pool of AMC-compliant, legally viable, and highly attractive job lines.
*   **Out of Scope**: The service does *not* assign these generated lines to specific individual workers based on their personal preferencing submissions. It also does *not* manage micro-rostering (e.g., specific shifts, 10-hour breaks).
...
*   **Affinity Matrix**: Rotational synergy (e.g., ICU + Anaesthetics) is scored using linear-scaled Pearson R coefficients derived from the MSIS survey data.
*   **Abstraction Strategy**: The survey data is abstracted into a standalone JSON configuration file, allowing the affinity weights to be hot-swapped or updated via future localized surveys without altering the core Python solver code.
*   **Diverse Preference Optimization (DivPO)**: The solver penalizes overlapping, identical elite job lines to prevent "clumping" and ensure a flatter, more diverse distribution of attractive rotations across all lines.
